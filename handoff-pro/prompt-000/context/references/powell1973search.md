...
When $\pmb{x}^{(n + 1)}$ has been found, another search is made along the first coor
...
dinate direction, and so the process continues iteratively changing the components of $x^{(k)}$ in sequence. We call this algorithm “the method that changes one variable at a time”.
