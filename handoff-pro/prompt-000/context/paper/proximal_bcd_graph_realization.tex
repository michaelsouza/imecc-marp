...
\begin{enumerate}
    ...
    \item We exhibit a Powell-type two-cycle for exact cyclic BCD on a one-dimensional DGP instance whose accumulation points are not stationary, as well as a planar tie-cycle illustrating the same nonunique-minimizer mechanism (Section~\ref{sec:experiments}).
...
\section{Counterexamples and Numerical Experiments}
\label{sec:experiments}

The first two subsections give exact counterexamples for the unregularized cyclic method.  We then show how the proximal term removes the tie-switching mechanism in the planar example.  The final subsection reports planar numerical experiments (\(K=2\)) for the proximal method.  In those experiments, each block subproblem is solved by a damped Newton method with backtracking line search; a new point is accepted only if it does not increase the proximal block objective, so the sufficient-decrease property holds in exact arithmetic.  The reported computations are therefore illustrative finite-precision realizations of the scheme analyzed in Sect.~\ref{sec:convergence}, which assumes exact block minimization.

\paragraph{Implementation details.}
The code is written in pure Python (standard library \texttt{random} module and NumPy, no compiled extensions).  The Powell-type counterexample below was also verified by the exact rational script \texttt{codes/powell\_type\_dgp\_cycle.py}.  The Newton solver uses at most 80 inner iterations with Armijo step acceptance; a fallback gradient step is used if the Hessian is indefinite.  The target instance is generated with seed \texttt{20260429}; the initial iterate for run $k$ of parameter $\mu$ uses seed $1000+\lfloor100\mu\rfloor+k$, with free coordinates drawn uniformly from $[-1.5,1.5]^2$.  Experiments were run on a laptop with an Intel Core i7-12700H CPU; a single run of 220 sweeps on the $n=14$ instance completes in under 2 seconds.

\subsection{A Powell-type cycle with nonstationary accumulation points}
\label{subsec:powell-cycle}

We now show that the anchored graph-realization objective can reproduce the essential pathology of Powell's coordinate-descent example~\cite{powell1973}: exact cyclic minimization can cycle with accumulation points that are not stationary.  Take \(K=1\), fix the single anchor \(x_1=0\), and let the graph be complete on \(V=\{1,2,3,4\}\).  The free variables are \(z=(x_2,x_3,x_4)\).  Set
\[
    d_{1i}^2=\frac95,\qquad i=2,3,4,
    \qquad
    d_{ij}^2=\frac{18}{5},\qquad 2\le i<j\le4.
\]
Consider the two free states
\[
    A=(-1,1,-1),\qquad
    B=(1,-1,1).
\]
At both states the objective has the same value,
\[
    F(A)=F(B)=\frac{38}{5},
\]
but the free gradients are
\[
    \nabla_z F(A)=\left(0,\frac85,0\right),
    \qquad
    \nabla_z F(B)=\left(0,-\frac85,0\right).
\]
Thus neither \(A\) nor \(B\) is stationary.

It remains to check that the cyclic exact coordinate method may legally move from \(A\) to \(B\) and back.  Suppose that, at a coordinate update, the other two free variables have values \(1\) and \(-1\).  This is exactly the situation at every inner step along the orbit below.  The one-dimensional block objective is then
\[
    \phi(t)
    =
    \frac12\left(t^2-\frac95\right)^2
    +
    \frac12\left((t-1)^2-\frac{18}{5}\right)^2
    +
    \frac12\left((t+1)^2-\frac{18}{5}\right)^2,
\]
and hence
\[
    \phi'(t)=6t(t^2-1).
\]
The only critical points are \(t=-1,0,1\), and
\[
    \phi(-1)=\phi(1)=\frac{172}{25}
    <
    \frac{419}{50}
    =
    \phi(0).
\]
Since \(\phi(t)\to\infty\) as \(|t|\to\infty\), the two values \(t=-1\) and \(t=1\) are global minimizers of every block subproblem visited by the cycle.  With cyclic order \(x_2,x_3,x_4\), a legal tie selection gives
\[
    (-1,1,-1)
    \longrightarrow
    (1,1,-1)
    ...
    (1,-1,1)
\]
and the symmetric choices from \(B\) return to \(A\).  Therefore the unregularized exact cyclic BCD method admits the two-cycle
\[
    A\longrightarrow B\longrightarrow A,
\]
and the two accumulation points of this cycle are nonstationary.

\subsection{A tie cycle without the proximal term}
\label{subsec:cycle}

We also exhibit a concrete planar instance on which exact cyclic BCD without regularization fails to converge.  Unlike the preceding example, the cycle points in this planar construction are stationary with respect to the free variables.  Its role is to make the tie geometry visible in \(K=2\).  The example uses two anchors only for this geometric transparency; the convergence theorem above concerns the one-anchor proximal method.  Fix \(x_1=(0,0)\) and \(x_2=(0,1)\), set all prescribed distances to one, and take the graph with edge set
\[
